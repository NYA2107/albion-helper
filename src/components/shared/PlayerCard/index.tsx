...
const usePlayerCard = () => {
  ...
  if (!context) {
    throw new Error(
      "PlayerCard compound components must be used within PlayerCard"
    );
  }
  return context;
};
...
const PlayerCard: FC<PlayerCardProps> & {
  ...
  Tags: React.FC;
  CreatedAt: React.FC;
  HeaderWrapper: React.FC<PropsWithChildren>;
} = (props) => {
  ...
  return (
    <PlayerCardContext.Provider value={props}>
      <Card>
        <CardContent>
          <Activity mode={children ? "visible" : "hidden"}>{children}</Activity>
  ...
  return (
    <div>
      <Button
        ...
        size="icon-xs"
        onClick={() => id !== undefined && onClickEdit && onClickEdit(id)}
      >
        <Pencil />
      </Button>
      <Button
        ...
        size="icon-xs"
        onClick={() => id !== undefined && onClickDelete && onClickDelete(id)}
      >
        <Trash />
      </Button>
    </div>
